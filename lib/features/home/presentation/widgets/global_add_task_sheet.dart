...
class GlobalAddTaskBottomSheet extends StatefulWidget {
  final List<SiteModel> sites;
  final List<SiteMemberModel> members;
  const GlobalAddTaskBottomSheet({super.key, required this.sites, required this.members});
  
  @override 
  State<GlobalAddTaskBottomSheet> createState() => _GlobalAddTaskBottomSheetState();
}

class _GlobalAddTaskBottomSheetState extends State<GlobalAddTaskBottomSheet> {
  final _formKey   = GlobalKey<FormState>();
  final _titleCtrl = TextEditingController();
  final _descCtrl  = TextEditingController();
  
  SiteModel? _selectedSite;
  SiteMemberModel? _selectedMember;
  ...
  void _submit() {
    ...
    if (_selectedMember == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please assign a member')));
      return;
    }
    ...
    return Padding(
      ...
      child: Form(key: _formKey, child: Column(mainAxisSize: MainAxisSize.min,
        ...
        children: [
          ...
          Text('Add new task globally', style: GoogleFonts.playfairDisplay(fontSize: 18, color: AppColors.textPrimary)),
          const SizedBox(height: 20),
          
          // SITE DROPDOWN
          DropdownButtonFormField<SiteModel>(
            value: _selectedSite,
            hint: Text('Select Site', style: GoogleFonts.lato(color: AppColors.textHint)),
            decoration: _dec('', '', Icons.location_city_outlined).copyWith(prefixIcon: null,
              contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14)),
            items: widget.sites.map((s) => DropdownMenuItem(
              value: s, child: Text(s.name, style: GoogleFonts.lato(fontSize: 13)),
            )).toList(),
          ...
          // DESCRIPTION INPUT
          TextFormField(controller: _descCtrl, maxLines: 2, textInputAction: TextInputAction.next,
          ...
          DropdownButtonFormField<SiteMemberModel>(
            ...
            hint: Text('Assign to member', style: GoogleFonts.lato(color: AppColors.textHint)),
            ...
            items: widget.members.map((m) => DropdownMenuItem(
              ...
              child: Row(children: [
                ...
                Text('${m.name} (${m.employeeId})', style: GoogleFonts.lato(fontSize: 13)),
              ]),
            )).toList(),
            ...
            SizedBox(width: double.infinity, height: 50,
              child: ElevatedButton(
                ...
                child: state.actionLoading
                    ...
                    : Text('Create Task', style: GoogleFonts.lato(fontSize: 15, fontWeight: FontWeight.w600, color: Colors.white)),
              ))),
        ])),
    );
  }
